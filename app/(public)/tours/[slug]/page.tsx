...
export default function PublicTourDetailPage() {
  ...
  const unitPrice = selectedOption?.price ?? tour?.basePrice ?? null;
  const totalPrice = unitPrice !== null ? unitPrice * guests : null;

  const submit = async () => {
    if (!selectedSlotId || !name || !guests) {
      setError('이름, 슬롯, 인원을 입력해주세요.');
      return;
    }
