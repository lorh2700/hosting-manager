...
export default function StaffPage() {
  ...
  const results = staff.filter(item => (filter === 'all' || item.roles.includes(filter)) && `${item.name} ${item.phone} ${item.email}`.toLowerCase().includes(query.trim().toLowerCase()));
  return <div className="mx-auto max-w-6xl space-y-7">
    ...
    {current && <StaffDetail key={`${current.key}:${current.role}`} staff={current} allStaff={staff} isAdmin={isAdmin} properties={properties} selfId={user?.id} onSaved={() => { void load(); void refreshProfile(); }} onClose={() => setSelected(null)} />}
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div className="flex flex-wrap gap-2" role="group" aria-label="직원 역할 필터">{(['all', 'admin', 'manager', 'cleaner'] as const).map(role => <button type="button" key={role} aria-pressed={filter === role} className={`${button} ${filter === role ? 'bg-stone-900 text-white' : 'bg-white'}`} onClick={() => setFilter(role)}>{role === 'all' ? '전체' : roles[role]} <span className="ml-1 opacity-60">{staff.filter(item => role === 'all' || item.roles.includes(role)).length}</span></button>)}</div><label className="text-sm">직원 검색<input type="search" placeholder="이름 · 연락처 · 이메일" value={query} onChange={event => setQuery(event.target.value)} className={field} /></label></div>
    {loading ? <p role="status" className="py-10 text-sm">직원 목록을 불러오는 중…</p> : <div className="space-y-3">{results.map(item => <button type="button" key={item.key} aria-expanded={selected === item.key} onClick={() => { setSelected(item.key); setCreating(false); setInvitations(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`grid w-full gap-3 border bg-white p-5 text-left sm:grid-cols-[1.2fr_1fr_1.4fr_auto] sm:items-center ${selected === item.key ? 'border-[var(--brand)]' : 'border-stone-200 hover:border-stone-400'}`}>
      ...
      <span className="text-sm">{roles[item.role]}</span><span className="text-sm text-stone-600">{item.scope === 'all' ? '전체 숙소' : item.propertyIds.map(id => properties.find(p => p.id === id)?.name || '조회 범위 밖 숙소').join(', ') || '배정 없음'}</span>
