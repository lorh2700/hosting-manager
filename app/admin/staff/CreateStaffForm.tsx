  ...
  return <section className="border border-stone-300 bg-white p-5 space-y-5">
    ...
    {role !== 'cleaner' ? <CreateUserForm embedded onSavingChange={setBusy} key={role} initial={{ email: '', role, propertyIds: [] }} properties={properties} onCreated={() => { setCreated(true); onCreated(); }} onClose={onClose} />
      : result ? <div className="space-y-4"><p>{name}님을 등록했습니다.</p><p className="text-sm text-stone-600">{result.initialPassword ? '로그인 정보를 담당자에게 전달해 주세요. 닫은 뒤에는 초기 비밀번호를 다시 표시하지 않습니다.' : '로그인 없이 일정 링크를 사용할 수 있습니다. 상세 화면에서 로그인 비밀번호를 발급할 수 있습니다.'}</p>
        ...
        <label className="block text-sm">담당 숙소<select value={mode} onChange={event => setMode(event.target.value as Scope)} className={field}><option value="none">배정 없음</option><option value="selected">선택한 숙소</option><option value="all">현재 관리 가능한 모든 숙소</option></select></label>
