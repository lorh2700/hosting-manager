...
const STATUS_ICONS: Record<IntegrationStatus, React.ReactNode> = {
  ...
  inactive: <Clock size={14} className="text-white/30" />,
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12">
      <header className="border-b border-white/10 pb-8 flex flex-col sm:flex-row gap-4 sm:justify-between sm:items-end">
        <div>
          <p className="text-[10px] tracking-[0.3em] text-white/50 mb-4">채널</p>
          <h1 className="text-3xl md:text-4xl font-light tracking-tight text-white">연동 관리</h1>
          <p className="text-white/50 mt-4 text-sm font-light tracking-wide">OTA 채널 연동 및 동기화 상태를 관리합니다.</p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={() => setShowAddForm(!showAddForm)}
            className="flex items-center gap-2 bg-white/10 text-white px-4 py-2.5 text-[11px] uppercase tracking-widest font-semibold hover:bg-white/20 transition-colors"
          >
            <Plus size={14} />
            연동 추가
          </button>
          <button
            ...
            disabled={syncing}
            className="flex items-center gap-2 bg-white text-black px-6 py-2.5 text-[11px] uppercase tracking-widest font-semibold hover:bg-white/90 transition-colors disabled:opacity-50"
          >
      ...
      {/* Add Form */}
      {showAddForm && (
        <form onSubmit={handleAdd} className="bg-[#111] border border-white/20 p-6 space-y-4">
          <h2 className="text-sm font-medium text-white mb-4">새 연동 추가</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-white/40 mb-2">숙소</label>
              <select
                value={newPropertyId}
                onChange={e => setNewPropertyId(e.target.value)}
                className="w-full bg-black/50 border border-white/10 text-white text-sm px-4 py-2.5 focus:outline-none focus:border-white/30"
              >
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-white/40 mb-2">채널</label>
              <select
                value={newProvider}
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-white/40 mb-2">연동 유형</label>
              <select
                value={newType}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-widest text-white/40 mb-2">iCal Import URL</label>
            <input
              type="url"
              value={newImportUrl}
              onChange={e => setNewImportUrl(e.target.value)}
              className="w-full bg-black/50 border border-white/10 px-4 py-2.5 text-sm text-white focus:outline-none focus:border-white/30"
              placeholder="https://www.airbnb.co.kr/calendar/ical/..."
            />
          </div>
          <div className="flex justify-end gap-3">
            <button type="button" onClick={() => setShowAddForm(false)} className="px-4 py-2 text-[10px] text-white/50 hover:text-white">
              취소
            </button>
            <button
              type="submit"
              disabled={adding}
              className="bg-white text-black px-6 py-2 text-[10px] tracking-widest font-semibold hover:bg-white/90 disabled:opacity-50"
            >
              {adding ? '추가 중...' : '추가'}
            </button>
          </div>
      ...
      {integrations.length === 0 ? (
        ...
        <div className="space-y-4">
          {integrations.map(integ => (
            <div key={integ.id} className={`bg-[#111] border p-5 transition-colors ${
              integ.status === 'error' ? 'border-red-500/30' :
              integ.status === 'inactive' ? 'border-white/5' :
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-white text-sm font-medium">{PROVIDER_LABELS[integ.provider]}</span>
                      <span className="text-white/30 text-xs">{integ.type.toUpperCase()}</span>
                      {integ.propertyName && (
                        <span className="text-white/40 text-xs">&middot; {integ.propertyName}</span>
                      )}
                    </div>
      ...
      {syncLogs.length > 0 && (
        <div>
          <h2 className="text-[10px] uppercase tracking-widest text-white/50 mb-4">최근 동기화 이력</h2>
              ...
              {syncLogs.slice(0, 20).map(log => (
                <div key={log.id} className="px-5 py-3 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3">
                    ...
                    {log.errorMessage && (
                      <span className="text-red-400/70 truncate max-w-[200px]">{log.errorMessage}</span>
                    )}
                  </div>
