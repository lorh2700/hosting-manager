    ...
    <div className="max-w-3xl mx-auto space-y-8 sm:space-y-10">
      <header className="flex flex-col sm:flex-row gap-4 sm:justify-between sm:items-start">
        ...
        <button
          onClick={handleSync}
          disabled={isSyncing}
          className="flex items-center gap-2 text-white/40 hover:text-white text-xs tracking-wide transition-colors disabled:opacity-50 shrink-0 active:scale-95 py-2"
        >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        ...
                        {cleaningStatus === 'done' ? (
                          ...
                          <span className="text-xs text-rose-400/70">청소 미배정</span>
                        )}
                      </div>
                    </div>
                    <span className="text-[10px] sm:text-[11px] bg-amber-500/10 text-amber-400/80 px-2.5 py-1.5 rounded-lg font-medium shrink-0">체크아웃</span>
                  </div>
                ))}
              </div>
            </div>
