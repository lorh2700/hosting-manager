...
DO $merge_users$
DECLARE
  source_id TEXT := 'staff-vFeH4skk3leswVgev6kZ';
  target_id TEXT := '6f53c2b2-2653-42ec-bf1a-0d2cc5b71077';
  src users%ROWTYPE;
  dst users%ROWTYPE;
  col RECORD;
  fk RECORD;
...
BEGIN
  ...
  IF src.role<>'cleaner' OR src.status<>'no_account' OR src.password<>''
    ...
    OR src.display_name IS DISTINCT FROM '도영'
    OR dst.display_name IS DISTINCT FROM 'lorh2700@gmail.com'
    OR dst.email<>'lorh2700@gmail.com' OR dst.role<>'admin' OR dst.status<>'active' THEN
  ...
  UPDATE users SET display_name='도영', phone=COALESCE(NULLIF(dst.phone,''),src.phone),
