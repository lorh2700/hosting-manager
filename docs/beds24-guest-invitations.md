...
- 새 초대장은 UUID와 만료 시각을 바이너리로 암호화한다. UUID 숙소 ID는 v2 형식(전체 URL 140자), 기존 영문·숫자 숙소 ID는 v3 형식(현재 숙소 기준 전체 URL 146자 이하)을 사용한다. 기존 JSON 암호화 링크도 계속 검증하며, 유효한 기존 링크는 동기화 시 유지한다. 손상된 링크는 적용 대상 예약의 다음 정상 동기화에서 새 링크로 교체한다.
