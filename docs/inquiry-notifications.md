...
- 지난 문의를 제외하고 새 문의부터 다시 시작할 때는 `node scripts/reset-inquiry-cutoff.mjs`로 변경 건수를 미리 확인한 뒤 `--apply`를 붙여 적용한다. 현재 활성화된 숙소의 기준 시각을 갱신하고, 그 이전 문의의 미발송 작업과 대기 알림을 제외한다. 발송 중인 작업이 있으면 변경하지 않고 중단한다. 기존 안내문과 대화별 직접 응대 상태는 유지한다.
